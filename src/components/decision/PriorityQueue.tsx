...
import { TONE_CLASSES, compositeScore, recommendation, verdictForLens } from "@/lib/decision/logic";
  ...
  return (
    <aside className="border-l border-border bg-sidebar h-full overflow-y-auto">
      <div className="px-6 py-5 border-b border-border">
        <div className="label-mono">Priority queue</div>
      </div>
      ...
      <ol className="divide-y divide-border">
        ...
        {ranked.length === 0 && (
          <li className="px-6 py-12 text-center">
            <p className="font-serif italic text-muted-foreground">No items yet. Add one to begin.</p>
          </li>
        )}
      </ol>
      ...
      {insightsOn && top3.length > 0 && (
        <div className="px-6 py-5 border-t border-border bg-background animate-fade-up">
          <div className="label-mono mb-4">Top recommendations</div>
